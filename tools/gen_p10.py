from cryptography.hazmat.primitives.asymmetric import rsa, ec, padding
from cryptography.hazmat.primitives import serialization, hashes
from cryptography.x509.oid import NameOID
from cryptography import x509
import base64
from enum import Enum
from gmssl import sm2, sm3, func
import binascii

class KeyType(Enum):
    RSA2048 = "RSA"
    SM2 = "SM2"

def get_signature_alg(key_type):
    if key_type == KeyType.RSA2048:
        return "SHA256withRSA"
    elif key_type == KeyType.SM2:
        return "SM3withSM2"
    else:
        raise RuntimeError("不支持的算法")

def string2_public_key(pub_key, key_type):
    try:
        decoded_key = base64.b64decode(pub_key)
        if key_type == KeyType.RSA2048:
            return serialization.load_der_public_key(decoded_key)
        elif key_type == KeyType.SM2:
            # 对于SM2，使用EC算法和BouncyCastle
            # sm2_obj = sm2.CryptSM2(
            #     public_key=binascii.hexlify(decoded_key).decode(),
            #     private_key=None
            # )
            # return sm2_obj
            return serialization.load_der_public_key(decoded_key)
        else:
            raise RuntimeError("不支持的算法")
    except Exception as e:
        raise RuntimeError(f"没有此类算法: {e}")

def string2_private_key(pri_key, key_type):
    try:
        decoded_key = base64.b64decode(pri_key)
        if key_type == KeyType.RSA2048:
            return serialization.load_der_private_key(decoded_key, password=None)
        elif key_type == KeyType.SM2:
            # 对于SM2，使用EC算法
            # sm2_obj = sm2.CryptSM2(
            #     public_key=None,
            #     private_key=binascii.hexlify(decoded_key).decode()
            # )
            # return sm2_obj
            return serialization.load_der_private_key(decoded_key, password=None)
        else:
            raise RuntimeError("不支持的算法")
    except Exception as e:
        raise RuntimeError(f"没有此类算法: {e}")

def gen_p10(pri_key, pub_key, key_type):
    # 第一步：根据算法将公钥字符串转换为公钥对象
    public_key = string2_public_key(pub_key, key_type)
    
    # 第二步：根据算法将私钥字符串转换为私钥对象
    private_key = string2_private_key(pri_key, key_type)
    
    try:
        # 第三步：生成X500Principal对象并创建签名
        x500_name = x509.Name([])  # 空的X500Principal
        
        # 第四步：创建PKCS10CertificationRequestBuilder
        builder = x509.CertificateSigningRequestBuilder()
        builder = builder.subject_name(x500_name)
        
        # 第五步：根据签名算法决定签名方式
        if key_type == KeyType.RSA2048:
            sign_algorithm = hashes.SHA256()
            # signature_algorithm = x509.RSASignatureWithSHA256()
        elif key_type == KeyType.SM2:
            sign_algorithm = hashes.SM3()
        else:
            raise RuntimeError("不支持的算法")
        
        # 第六步：构建CSR请求并进行签名
        csr = builder.sign(private_key, sign_algorithm)
        
        # 获取DER格式的CSR并进行base64编码
        csr_der = csr.public_bytes(serialization.Encoding.DER)
        return base64.b64encode(csr_der).decode('utf-8')
    
    except Exception as e:
        raise RuntimeError(f"生成P10请求失败: {e}")
    

# from pyasn1.type import char, namedtype, namedval, univ, constraint, tag, useful
# from pyasn1.codec.der import encoder
# import base64
# from gmssl import sm2, sm3
# import binascii

# # ASN.1结构定义 - 用于PKCS#10证书请求
# class AttributeType(univ.ObjectIdentifier):
#     pass

# class AttributeValue(univ.Any):
#     pass

# class AttributeTypeAndValue(univ.Sequence):
#     componentType = namedtype.NamedTypes(
#         namedtype.NamedType('type', AttributeType()),
#         namedtype.NamedType('value', AttributeValue())
#     )

# class RelativeDistinguishedName(univ.SetOf):
#     componentType = AttributeTypeAndValue()

# class RDNSequence(univ.SequenceOf):
#     componentType = RelativeDistinguishedName()

# class Name(univ.Choice):
#     componentType = namedtype.NamedTypes(
#         namedtype.NamedType('', RDNSequence())
#     )

# class AlgorithmIdentifier(univ.Sequence):
#     componentType = namedtype.NamedTypes(
#         namedtype.NamedType('algorithm', univ.ObjectIdentifier()),
#         namedtype.OptionalNamedType('parameters', univ.Any())
#     )

# class SubjectPublicKeyInfo(univ.Sequence):
#     componentType = namedtype.NamedTypes(
#         namedtype.NamedType('algorithm', AlgorithmIdentifier()),
#         namedtype.NamedType('subjectPublicKey', univ.BitString())
#     )

# class CertificationRequestInfo(univ.Sequence):
#     componentType = namedtype.NamedTypes(
#         namedtype.NamedType('version', univ.Integer()),
#         namedtype.NamedType('subject', Name()),
#         namedtype.NamedType('subjectPKInfo', SubjectPublicKeyInfo()),
#         namedtype.NamedType('attributes', 
#             univ.SetOf(componentType=univ.Any()).subtype(
#                 implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatConstructed, 0)
#             )
#         )
#     )

# class CertificationRequest(univ.Sequence):
#     componentType = namedtype.NamedTypes(
#         namedtype.NamedType('certificationRequestInfo', CertificationRequestInfo()),
#         namedtype.NamedType('signatureAlgorithm', AlgorithmIdentifier()),
#         namedtype.NamedType('signature', univ.BitString())
#     )

# def gen_p10(pri_key, pub_key, key_type):
#     """
#     生成p10请求文件
    
#     Args:
#         pri_key: 私钥字符串（Base64编码）
#         pub_key: 公钥字符串（Base64编码）
#         key_type: 密钥类型（SM2）
    
#     Returns:
#         Base64编码的P10请求
#     """
#     # 解码Base64字符串
#     pri_key_bytes = base64.b64decode(pri_key)
#     pub_key_bytes = base64.b64decode(pub_key)
    
#     # 创建SM2对象
#     sm2_crypt = sm2.CryptSM2(private_key=binascii.hexlify(pri_key_bytes).decode(), 
#                              public_key=binascii.hexlify(pub_key_bytes).decode())
    
#     # 创建空的X500Name
#     subject = Name()
    
#     # 创建公钥信息
#     algorithm_id = AlgorithmIdentifier()
#     algorithm_id['algorithm'] = univ.ObjectIdentifier('1.2.156.10197.1.301')  # SM2算法OID
    
#     spki = SubjectPublicKeyInfo()
#     spki['algorithm'] = algorithm_id
#     # 正确创建BitString对象
#     spki['subjectPublicKey'] = univ.BitString(hexValue=binascii.hexlify(pub_key_bytes).decode())
    
    
#     # 创建证书请求信息
#     cert_req_info = CertificationRequestInfo()
#     cert_req_info['version'] = univ.Integer(0)
#     cert_req_info['subject'] = subject
#     cert_req_info['subjectPKInfo'] = spki
#     cert_req_info['attributes'] = univ.SetOf(componentType=univ.Any())
    
#     # DER编码请求信息
#     cert_req_info_der = encoder.encode(cert_req_info)
    
#     # 使用SM3算法对请求信息进行哈希
#     sm3_hash = sm3.sm3_hash(func.bytes_to_list(cert_req_info_der))
#     sm3_hash_bytes = bytes.fromhex(sm3_hash)
    
#     # 使用SM2私钥对哈希值进行签名
#     signature_bytes = bytes.fromhex(sm2_crypt.sign(sm3_hash_bytes.hex()))
    
#     # 创建签名算法标识符
#     sig_algorithm = AlgorithmIdentifier()
#     sig_algorithm['algorithm'] = univ.ObjectIdentifier('1.2.156.10197.1.501')  # SM3withSM2的OID
    
#     # 创建完整的证书请求
#     cert_req = CertificationRequest()
#     cert_req['certificationRequestInfo'] = cert_req_info
#     cert_req['signatureAlgorithm'] = sig_algorithm
#     cert_req['signature'] = univ.BitString.fromOctets(signature_bytes)
    
#     # DER编码整个证书请求并进行Base64编码
#     cert_req_der = encoder.encode(cert_req)
#     return base64.b64encode(cert_req_der).decode('ascii')

if __name__ == "__main__":
    # pri_key = ""
    # pub_key = ""
    # key_type = KeyType.RSA2048
    pri_key = "MIGEAgEAMBAGByqGSM49AgEGBSuBBAAKBG0wawIBAQQgDQKlS7bO/Kk5ki6EX2jc7fwpBZdQSfLLkydhhpfNJp+hRANCAASvzBZG6h3rpDOLy9Fx5yW3Pa6Od3CngeFK5f8uUlPHrtxLmNl0CHBserrsk/fFJanzIKpEEIisR7AOykJ2wqgr"
    pub_key = "MFYwEAYHKoZIzj0CAQYFK4EEAAoDQgAEr8wWRuod66Qzi8vRcecltz2ujndwp4HhSuX/LlJTx67cS5jZdAhwbHq67JP3xSWp8yCqRBCIrEewDspCdsKoKw=="
    key_type = KeyType.SM2
    print(gen_p10(pri_key=pri_key, pub_key=pub_key, key_type=key_type))
    