    ...
    try:
        ...
        if pfxCert and os.path.exists(pfxCert):
            ...
            if pfx_result['key_type'] != algorithm:
                result['message'] = f"警告：PFX证书中检测到的算法类型({pfx_result['key_type']})与指定的算法类型({algorithm})不匹配"
        elif pfxCert:
        ...
        if pubCert and os.path.exists(pubCert):
            ...
            if pub_result['key_type'] != algorithm:
                if 'warning' in result['message']:
                    result['message'] += f"，CER证书中检测到的算法类型({pub_result['key_type']})与指定的算法类型({algorithm})不匹配"
                else:
                    result['message'] = f"警告：CER证书中检测到的算法类型({pub_result['key_type']})与指定的算法类型({algorithm})不匹配"
        elif pubCert:
...
def parse_key_from_certificate(cert_path, password=None):
    """
